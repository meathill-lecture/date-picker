...
var _EasyDate = __webpack_require__(8);
...
var DatePicker = function () {
  ...
  function DatePicker(target) {
    ...
    this.target = target;
    this.range = _utils2.default.getRange(options);
  ...
  _createClass(DatePicker, [{
    ...
    value: function confirm() {
      var value = this.$el.find('.select').map(function () {
        return (0, _jquery2.default)(this).data('date');
      }).get();
    ...
    value: function setValue(value) {
      ...
      values.forEach(function (value) {
        _this2.$el.find('[data-date="' + value + '"]').addClass('select');
      });
    }
  }, {
    key: 'show',
    value: function show() {
      ...
      var range = _utils2.default.getRange(options);
      if (range !== this.range) {
        this.$el.remove();
        this.createElement(options);
        this.delegateEvent(options);
...
var _RangeDatePicker = __webpack_require__(9);
...
var RangeDatePicker = function (_DatePicker) {
  _inherits(RangeDatePicker, _DatePicker);

  function RangeDatePicker() {
    _classCallCheck(this, RangeDatePicker);

    return _possibleConstructorReturn(this, (RangeDatePicker.__proto__ || Object.getPrototypeOf(RangeDatePicker)).apply(this, arguments));
  }

  _createClass(RangeDatePicker, [{
    key: 'confirm',
    value: function confirm() {
      var start = this.$el.find('.start').data('date');
      var end = this.$el.find('.end').data('date');
      this.start.value = start + ' 00:00';
      this.end.value = end + ' 23:59';
      this.target.val(start + ' ~ ' + end);
      var event = document.createEvent('HTMLEvents'); // 为了让 Vue 能接收到事件
    ...
    value: function createElement(options) {
      this.counter = 0;
      _get(RangeDatePicker.prototype.__proto__ || Object.getPrototypeOf(RangeDatePicker.prototype), 'createElement', this).call(this, options);

      var targetName = this.target.attr('name');
      this.start = this.createInput(targetName, 'start');
      this.end = this.createInput(targetName, 'end');
    }
  }, {
    key: 'createInput',
    value: function createInput(targetName, pos) {
      var input = document.createElement('input');
      input.type = 'hidden';
      input.name = targetName + '-' + pos;
      (0, _jquery2.default)(input).insertAfter(this.target);
      return input;
    }
  }, {
    key: 'createMonthObject',
    value: function createMonthObject(current, today, start, end) {
      ...
      var month = _get(RangeDatePicker.prototype.__proto__ || Object.getPrototypeOf(RangeDatePicker.prototype), 'createMonthObject', this).call(this, current, today, start, end);
      month.days = month.days.map(function (item, i) {
        item['index'] = _this2.counter + i;
        return item;
      });
    ...
    value: function setValue(value) {
      ...
      var end = max(values);
      var start = min(values);
      start = this.$el.find('[data-date="' + start + '"]').addClass(function () {
        var classes = ['start'];
        if (end > start) {
          classes.push('tails');
        }
    ...
    key: 'onClick',
    value: function onClick(event) {
      var start = this.$el.find('.start');
      var end = this.$el.find('.end');
      ...
      var startIndex = start.data('index');
      ...
      if (startIndex <= index) {
        ...
        start.removeClass('start tails').addClass('end');
        li.addClass('select start tails');
      }
      this.$el.find('li[data-index]').slice(Math.min(startIndex, index), Math.max(startIndex, index)).addClass('select');
      end = li;
...
module.exports = {
  getRange: function getRange(options) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return ((stack1 = container.invokePartial(__webpack_require__(4),depth0,{"name":"calendar","data":data,"indent":"      ","helpers":helpers,"partials":partials,"decorators":container.decorators})) != null ? stack1 : "");
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
